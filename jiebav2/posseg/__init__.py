...
class POSTokenizer(object):
    ...
    def __init__(self, tokenizer=None):
        self.tokenizer = tokenizer or jiebav2.Tokenizer()
...
def cut(sentence, HMM=True):
    ...
    """
    global dt
    if jiebav2.pool is None:
        for w in dt.cut(sentence, HMM=HMM):
            yield w
    else:
        parts = strdecode(sentence).splitlines(True)
        if HMM:
            result = jiebav2.pool.map(_lcut_internal, parts)
        else:
            result = jiebav2.pool.map(_lcut_internal_no_hmm, parts)
        for r in result:
            for w in r:
                yield w
